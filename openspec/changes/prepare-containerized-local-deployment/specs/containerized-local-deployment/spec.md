...
A documentação operacional SHALL fornecer comandos fail-closed, inclusive sob otimização Python, que rejeitem qualquer porta publicada, capability adicionada, device, mount, comando ou entrypoint fora da whitelist, MUST NOT criar arquivos no workspace e SHALL distinguir o estado de health do Docker de uma requisição HTTP independente. Toda operação Compose, inclusive CID e rollback, MUST fixar a raiz validada do repositório, o caminho absoluto de `compose.yaml` e o nome de projeto `spock-workspace-dashboard`, neutralizando variáveis Compose herdadas. O CLI MUST usar Bash absoluto em modo privilegiado, desativar tracing e neutralizar `BASH_ENV`/`ENV` antes de ler a chave; depois MUST capturar a chave em variável não exportada e exportar `HERMES_API_KEY=` antes de qualquer filho; somente o processo exato `up --no-start` MAY receber a chave salva. Depois da canonicalização, device/inode e condição não-symlink MUST ser revalidados imediatamente antes desse `up`; o contêiner MUST permanecer parado durante inspeção efetiva, que MUST confirmar exatamente o entrypoint gate e o comando Node esperados, e MUST verificar no entrypoint a identidade device/inode do bind consumido antes de executar a aplicação. Overrides Compose de command/entrypoint MUST ser rejeitados. Mismatch ou falha posterior MUST remover o contêiner staged e MUST NOT alcançar startup bem-sucedido. A documentação MUST limitar compatibilidade a Linux Docker local que preserve identidade do bind, declarar que não há garantia atômica host/daemon e falhar fechado em remapeamento incompatível.
