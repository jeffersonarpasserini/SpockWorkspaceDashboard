...
- A execução Docker real anterior confirmou build, criação/start, health e o verifier então existente, mas antecede estas novas assertions de UID/rootfs/workspace. Portanto a tarefa de host permanece pendente até uma nova execução da versão corrigida; testes com mocks não substituem essa evidência.
