...
Só então executa `pull` sem chave, `up --no-start --no-build` com a chave opcional, inspeção efetiva ainda sem processo e `start` sem chave. O gate Linux device/inode executa antes do Node; mismatch, health inválido ou liveness inválida remove o contêiner staged e não registra sucesso. Isto não promete atomicidade host/daemon: o gate valida o mount que o daemon efetivamente fixou. Daemon remoto, Docker Desktop ou filesystem que remapeie device/inode falha fechado e não é suportado sem validação equivalente. `DEPLOY_HEALTH_TIMEOUT` e `DEPLOY_HEALTH_INTERVAL` devem ser inteiros positivos. Sucesso grava apenas versão e referência imutável (sem secret) em state local com modo restrito. Temporários são removidos em saída e sinais.
